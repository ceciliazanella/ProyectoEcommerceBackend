...
export default class CartDAO {
    ...
    try {
      ...
      carts.forEach((cart) => {
        cart.products.forEach((item) => {
          if (!item.product) {
            console.log("❌ Este Producto no es válido...:", item);
          }
        });
      });
      ...
      console.error(
        "❌ Error al querer obtener los Carritos...:",
        error.message
      );
  ...
  async insertOne() {
    try {
      ...
      throw new ErrorManager(
        "❌ Error al querer Crear tu Chococarrito...",
        500
      );
    }
  }
  ...
  async getCartProducts(cid, searchTerm = "") {
    try {
      ...
      throw new ErrorManager(
        "❌ Error al querer obtener los Productos del Carrito...",
        500
      );
    }
  }
  ...
  async emptyCart(cid) {
    try {
      ...
      throw new ErrorManager(
        "❌ Hubo un Error al querer Vaciar el Carrito...",
        500
      );
    }
  }
  ...
  async #validateProductInCart(cid, pid, quantity) {
    if (!mongoose.Types.ObjectId.isValid(pid)) {
      throw new ErrorManager("❌ ID de Producto inválido...", 400);
    }
